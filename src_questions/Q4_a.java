...
public class Q4_a {
    ...
    public int get(int key) {
        ...
        // In O(1) time, remove the element from the current frequency list and move it to the next one.
    ...
    public void put(int key, int value) {
        if (!valueMap.containsKey(key) && size > 0){
            ...
            if (valueMap.size() == size) {
                // Take out the first node (LRU) from the list of lowest frequency nodes (LFU)
    ...
    public static void main(String[] args) { // Printing all cache
        Q4_a cache = new Q4_a(2);
        cache.put(1, 1);
        cache.put(2, 2);
